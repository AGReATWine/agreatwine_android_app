...
class WineDetailsScreen extends StatelessWidget {
  ...
  Future<List<Map<String, dynamic>>> _getWineDetails(
    ...
    if (results[0]['SLC'] != '') {
      slcList =
          slcEntries.map<double>((entry) => entry['Price'] as double).toList();
    } else {
      slcList;
    }
    ...
    if (results[0]['TLC'] != '') {
      tlcList =
          tlcEntries.map<double>((entry) => entry['Price'] as double).toList();
    } else {
      tlcList;
    }
    ...
    for (final ratingYear in pairRatingsPrice.keys) {
      ...
      medianPricesList
          .add({'RatingYear': ratingYear, 'MedianPrice': medianPrice});
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        future:
            _getWineDetails(wineDetails['fullName'], wineDetails['wineryName']),
        builder: (BuildContext context,
            AsyncSnapshot<List<Map<String, dynamic>>> snapshot) {
          if (snapshot.hasData) {
            final details = snapshot.data!;
            final vintages =
                details.map((detail) => detail['Vintage'].toString()).toList();
            final evaluations =
                details.map((detail) => detail['EvaluationAvg']).toList();
            final scoreAvg =
                details.map((detail) => detail['ScoreAvg']).toList();
            final tasting = details.map((detail) => detail['Tasting']).toList();
            final grapes = details.map((detail) => detail['Grapes']).toList();
            final agingMonths =
                details.map((detail) => detail['AgingMonths']).toList();
            final agingType =
                details.map((detail) => detail['AgingType']).toList();
            ...
            final xMin = xAxisYears
                .reduce((value, element) => value < element ? value : element);
            final xMax = xAxisYears
                .reduce((value, element) => value > element ? value : element);

            final yMin = appellationPricesList
                .reduce((value, element) => value < element ? value : element);
            final yMax = appellationPricesList
                .reduce((value, element) => value > element ? value : element);
            ...
            return SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  _buildVintageWidget(
                      vintages, evaluations, scoreAvg, tasting, context),
                  _buildChartWidget(
                      xMin.toDouble(),
                      xMax.toDouble(),
                      yMin.toDouble(),
  ...
  Widget _buildDetailsWidget(
      BuildContext context, Map<String, dynamic> wineDetails) {
    return Column(children: [
      Container(
      ...
      Container(
        decoration: BoxDecoration(
            border: Border(
                bottom: BorderSide(width: 0.5, color: Colors.grey.shade600))),
            ...
            children: [
              ...
              TableRow(
                children: [
                  ...
                  Row(
                    children: [
                      Text(
                          '${wineDetails['appellationLevel']}'
                          ' '
                          '${wineDetails['appellationName']}'
                          '  ',
                          softWrap: true),
                      GestureDetector(
                        onTap: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) => EntriesScreen(
                                  level: wineDetails['appellationName'],
                                  levelName: 'DOC'),
                            ),
                          );
                        },
              ...
              TableRow(
                children: [
                  Text('Aging'),
                  Text(
                      '${wineDetails['agingType']} | ${wineDetails['agingMonths']} months'),
                ],
              ),
              TableRow(
                children: [
  ...
  Widget _buildScoreWidget(
      Map<String, dynamic> wineDetails, slcCount, tlcCount) {
    return Column(
      children: [
        Container(
          margin: EdgeInsets.only(top: 10, bottom: 0),
                      ...
                      if (snapshot.hasData) {
                        return Row(
                          children: [
                            ...
                            Text(' '),
                            Text(wineDetails['rank'].toInt().toString(),
                                style: TextStyle(
                                    color: Utils.getScoreColor(
                                        ((wineDetails['rsScore'] -
                                                wineDetails['rank'] +
                        ...
                        if (snapshot.hasData) {
                          return Row(
                            children: [
                              ...
                              Text(' '),
                              Text(wineDetails['rank2'].toInt().toString(),
                                  style: TextStyle(
                                      color: Utils.getScoreColor(
                                          ((wineDetails['rs2Score'] -
                                                  wineDetails['rank2'] +
                        ...
                        if (snapshot.hasData) {
                          return Row(
                            children: [
                              ...
                              Text(' '),
                              Text(wineDetails['rank3'].toInt().toString(),
                                  style: TextStyle(
                                      color: Utils.getScoreColor(
                                          ((wineDetails['rs3Score'] -
                                                  wineDetails['rank3'] +
                      ...
                      if (snapshot.hasData) {
                        return Row(
                          children: [
                            ...
                            Text(' '),
                            Text(wineDetails['qprank'].toInt().toString(),
                                style: TextStyle(
                                    color: Utils.getScoreColor(
                                        ((wineDetails['qpScore'] -
                                                wineDetails['qprank'] +
  ...
  Widget _buildVintageWidget(List vintages, List evaluations, List scoreAvg,
      List tasting, BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Container(
                            ...
                            onPressed: () {
                              showModalBottomSheet(
                                ...
                                builder: (BuildContext context) {
                                  return Container(
                                    padding:
                                        EdgeInsets.all(16.0), // add padding
                                    child: tasting[i] == ''
                                        ? Text(
