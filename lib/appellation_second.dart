...
class _EntriesScreenState extends State<EntriesScreen> {
  ...
  Future<List<Map<String, dynamic>>> searchEntries() async {
    ...
    Database database = await openDatabase(path);
    List<Map<String, dynamic>> results = await database.rawQuery(
      "SELECT * FROM allwines WHERE SLC = ? AND Entry = 1 AND FullName LIKE ?",
      [widget.slc, '%${_searchController.text}%'], // filter by the search text field
    );
    await database.close();
    return results;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("${widget.slc} Appellation Wine List"),
      ),
      body: Column(
        children: [
          Expanded(
